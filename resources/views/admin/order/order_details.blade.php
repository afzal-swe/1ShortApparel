...
<div class="modal-body">
    ...
    <div class="row">
        <div class="col-4">
         ...
         <div class="col-4">
             <p>Subtotal : {{ $order->subtotal }} {{ $settings->currency }}</p>
         </div>
         <div class="col-4">
             <p>Total : {{ $order->total }} {{ $settings->currency }}</p>
         </div><br>
                   ...
                   <tbody>
                    @foreach($order_details as $row)
                     <tr>
                       ...
                       <td>{{ $row->size }}</td>
                       <td>{{ $row->color }} </td>
                       <td>{{ $row->quantity }} x {{ $row->single_price }} {{ $settings->currency }}</td>
                       <td>{{ $row->subtotal_price }} {{ $settings->currency }}</td>
                     </tr>
                    @endforeach 
                   </tbody>
                 </table>
